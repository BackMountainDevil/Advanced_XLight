...
def get_att_eng(memo_rl):
    """
    获取实验结果（从环境引擎获取的att），取测试集最后10轮的结果的平均值
    """
    records_dir = os.path.join("records", memo_rl)
    ...
    for traffic_file in traffic_files:
        ...
        # print(f'mean_att:{np.mean(last_10_att)} {traffic_file} last_10_att:{last_10_att}')
